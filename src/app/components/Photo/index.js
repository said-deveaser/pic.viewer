...
const Photo = (props) => {
    ...
    const { id } = useParams();
    return props.state.photos.map(photo => {
        if (photo.id === id) {
            return (
                <div key={photo.id} className={`${css['container']} ${css['app-page']}`}>
                        ...
                        <div className={`${css['post']} ${css['gallery__ite$mcss']}`}>
                            ...
                            <div className={css["post__description"]}>
                                <div className={css["post__information"]}>
                                    <a target="_blanc" href={photo.links.html} className={`${css["post__publish-date"]} ${css["publish-link"]}`}>
